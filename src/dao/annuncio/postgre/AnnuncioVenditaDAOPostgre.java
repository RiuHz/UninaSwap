...
class AnnuncioVenditaDAOPostgre extends DatabaseManager {
	...
	public ArrayList<AnnuncioDTO> getAnnunciVenditaRecenti(UtenteDTO utente) throws SQLException {
		...
		while (result.next()) {
			AnnuncioDTO annuncio = new AnnuncioVenditaDTO(
	...
	public void creaAnnuncioVendita(ProdottoDTO prodotto, String consegna, double prezzo) throws SQLException {
		...
		String query = "INSERT INTO Annuncio_Vendita (Consegna, Prezzo, Stato, ID_Prodotto) VALUES (?, ?, 'Attivo', ?)";
		
        PreparedStatement statement = db.prepareStatement(query);
        statement.setString(1, consegna);
        statement.setDouble(2, prezzo);
        statement.setInt(3, prodotto.getId());
